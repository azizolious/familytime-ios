...
//------------------OLD CODE-------//-----COMMENTED TO RESOLVE START END TIME-----//
...
open class TenClock : UIControl{
    ...
    @IBInspectable var insetAmount: CGFloat = 30
    var internalShift: CGFloat = 5;
    var pathWidth:CGFloat = 30
    ...
    open var firstTime = true
    open var hoursIntt = 00
    open var minutesIntt = 00
    ...
    var disableSuperviewScroll = true
    ...
    open var headText: String = "".localized
    open var tailText: String = "".localized
...
//        return calendar.dateByAddingComponents(comps, toDate: Date().startOfDay as Date, options: .init(rawValue:0))!
    ...
    func timeToAngle(_ date: Date) -> Angle{
        ...
        return medStepFunction(CGFloat(M_PI_2 - ( min / (12 * 60)) * 2 * Double.pi), stepSize: CGFloat( 2 * Double.pi / (12 * 60 / 5)))
    }

    // input an angle, output: 0 to 4pi
    func angleToTime(_ angle: Angle) -> Date{
        let dAngle = Double(angle)
        let min = CGFloat(((M_PI_2 - dAngle) / (2 * Double.pi)) * (12 * 60))
    ...
    open func update1(hours:Int,mins:Int) {
        ...
        updateWatchFaceTicks()
        updateWatchFaceNumerals()
        //updateWatchFaceTitle1()
        let f = UIFont.BoldFont(size: (UI_USER_INTERFACE_IDIOM() == .pad) ? .size16 : .size23)
//        let f = UIFont.BoldFont(size: .size23)
        let cgFont = CTFontCreateWithName((f.fontName as CFString?)!, f.pointSize,nil)
//        let titleTextLayer = CATextLayer()
        titleTextLayer.bounds.size      = CGSize( width: titleTextInset.size.width, height: 45)
        ...
        titleTextLayer.string = String(format: "%02ld"+"time_hours".localized+" : %02d"+"time_mintues".localized, hours, mins)
        CATransaction.commit()
...
//        fiveMinIncrements = fiveMinIncrements+1
...
//        titleTextLayer.string = "\(fiveMinIncrements / 12)hr \((fiveMinIncrements % 12) * 5)min"
        ...
        let minutes = (fiveMinIncrements % 12) * 5
        if firstTime == true{
            titleTextLayer.string = String(format: "%02ld"+"time_hours".localized+" : %02d"+"time_mintues".localized, hoursIntt, minutesIntt)
        } else {
            titleTextLayer.string = String(format: "%02ld"+"time_hours".localized+" : %02d"+"time_mintues".localized, Int(hours), minutes + 5)
        }
        
        titleTextLayer.position = self.layer.center
        print("mus");
//        firstTime = false

        print(titleTextLayer.string ?? "");
    }
