...
- (id)showDropDown:(UIButton *)b height:(CGFloat *)height array:(NSArray *)arr tableHeight:(int)tableHeight direction:(NSString *)direction {
    ...
    if (self) {
       ...
       // table.backgroundColor = [UIColor colorWithRed:0.239 green:0.239 blue:0.239 alpha:1];
        ...
        table.frame = CGRectMake(0, 0, btn.size.width, tableHeight);//*height);
        [UIView commitAnimations];
        [b.superview addSubview:self];
       [self addSubview:table];
        [b.superview bringSubviewToFront:table];
...
//    [btnSender addSubview:imgView];
    selectedUser = [[list objectAtIndex:indexPath.row] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
    [self myDelegate];
}
