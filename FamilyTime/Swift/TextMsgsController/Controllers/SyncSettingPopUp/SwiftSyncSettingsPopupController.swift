//
//  SwiftSyncSettingsPopupController.swift
//  FamilyTime
//
...
class SwiftSyncSettingsPopupController: UIViewController, UIAlertViewDelegate {
    ...
    func setupUI() {
        ...
        syncButton = setupSyncButton() ?? UIButton()
        syncButton.addTarget(self, action: #selector(handleSync(_:)), for: .touchUpInside)
    ...
    @objc func handleSync(_ sender: Any?) {
        synSettings()
    }
...
//        }
        ...
        SwiftFTUtils.showHUDAdded(to: view, withText: "Loading...".myModification(), animated: true)
        let url = String(format: "\(kSyncSettings)/\(Int(child_Id ?? "") ?? -1)")
        ...
        ApiManager.shared().mesh_postApi(withOutParam: url) { json, errorCode, message in
            DispatchQueue.main.async {
                ...
                if status != 200 {
                    ...
                    let alert = UIAlertView(title: "Error!".localized, message: message ?? "", delegate: self, cancelButtonTitle: "OK".localized)
                    alert.show()
                } else {
                    let message = json["message"] as? String
                    let alert = UIAlertView(title: "Sync Settings".localized, message: message ?? "", delegate: self, cancelButtonTitle: "OK".localized)
                    alert.show()
                    UserDefaults.standard.set("YES", forKey: "gobacknow")
                    UserDefaults.standard.synchronize()
                }
    ...
    func setupLaterButton() -> UIButton? {
        ...
        button.setTitleColor(UIColor.white, for: .normal)
        button.backgroundColor = CommonModel.color(fromHexString: color)
        button.setTitle("Later".myModification(), for: .normal)
    ...
    func setupMessageLabel() -> UILabel? {
        ...
        if IS_IPHONE_4() {
            ...
            label = UILabel(frame: CGRect(x: 25.0, y: (imageView.frame.maxY) + 15.0, width: view.bounds.width - 50.0, height: cancelButton.frame.minY - (imageView.frame.maxY) - 15.0))
        ...
        let rect = finalMessage.boundingRect(with: CGSize(width: (label?.bounds.width)!, height: 10000), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
